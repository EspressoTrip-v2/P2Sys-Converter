...
def system_template_fn(
    ...
    IP = pd.DataFrame(index=np.arange(0, len(customer_pricelist)), columns=IPcols)
    ...
    PLTA = pd.DataFrame(index=np.arange(0, len(customer_pricelist)), columns=PLTAcols)
    ...
    PPC = pd.DataFrame(index=np.arange(0, len(customer_pricelist)), columns=PPCcols)
    ...
    PD = pd.DataFrame(index=np.arange(0, len(customer_pricelist)), columns=PDcols)
    ...
    with pd.ExcelWriter(file_directory, engine="xlsxwriter") as writer:
        ...
        PLTA.to_excel(writer, sheet_name="Price_List_Tax_Authorities", index=False)
        ...
        workbook.define_name("Item_Pricing", f"=Item_Pricing!$A$1:$K${IP.shape[0]+4}")
        workbook.define_name(
            "Price_List_Tax_Authorities",
            f"=Price_List_Tax_Authorities!$A$1:$K${PLTA.shape[0]+4}",
        )
        workbook.define_name(
            "Pricing_Price_Checks", f"=Pricing_Price_Checks!$A$1:$K${PPC.shape[0]+4}"
        )
        workbook.define_name(
            "Item_Pricing_Details", f"=Item_Pricing_Details!$A$1:$K${PD.shape[0]+4}"
        )
